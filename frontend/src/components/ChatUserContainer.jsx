...
const ChatUserContainer = () => {
  ...
  const handleSelectUser = async (item) => {
    if (userInfo) {
      ...
      const filtered1 = myArrchat[0]
        .replace(/\D+/g, "")
        .split("")
        .map((e) => parseInt(e));
      const filtered2 = myArrcurren[0]
        .replace(/\D+/g, "")
        .split("")
        .map((e) => parseInt(e));
      ...
      const combinedId =
        reduced1 > reduced2 ? loggedUser + newUser : newUser + loggedUser;
  ...
  return (
    <>
      ...
      <div
        style={{ height: "250px", overflowY: "scroll", scrollbarWidth: "none" }}
      >
        {users?.map((u) => {
          return (
            <>
